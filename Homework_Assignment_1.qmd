...
```{r}
# Get the length of the list of unique gene ids in the dataframe
...
The genes with the largest magnitude fold change tend to have the smallest p-values (small p-value -> large -log(p-value) because large changes in expression are typically significant. 

There might be a mismatch between the magnitude of fold change and the p-values because p-values result from both variance *and* consistency. Small fold changes can be highly significant if the difference is consistent across samples. Similarly, large fold changes can be insignificant if the difference is highly variable or inconsistent across samples. 
...
# Reference the dataset
DE_results_msn24_EtOH %>%
  # Get all the rows where the word phosphoglucomutase is present in the gene annotation 
  filter(grepl("phosphoglucomutase", annotation, ignore.case = TRUE)) %>%
  
  # Sort the dataframe by common name
  arrange(common_name) %>%
  
  # Get only the relevant columns: common name and EtOH response
  select(common_name, 
...
PGM1 is mildly downregulated in both the WT ethanol response and the mutant ethanol response. PGM2 is highly upregulated in the WT ethanol response but only mildly upregulated in the mutant ethanol response. This indicates that the PGM2 isoform of phosphoglucomutase is normally upregulated during ethanol stress, a response that is not present to the same extent in the mutant. Based on this information, the *msn2/4*ΔΔ mutation is therefore associated with an impaired ethanol stress response and *could be* (but is not definitevly) responsible for the difference. 
